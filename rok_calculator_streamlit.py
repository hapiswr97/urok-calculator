...
with tab1:
    ...
    with col3:
        ...
        if st.button("🔁 Switch Customer"):
            if not selected_customer:
                ...
                st.session_state.form_key = f"form_{selected_customer}"
    ...
    if st.button("⚙️ Calculate"):
        if not before_choice or not after_choice:
            ...
            differences = {k: after[k] - before[k] for k in before.keys() if k != "timestamp"}
            ...
            result = {
                ...
                "monster_gems": monster_gems,
                "honor_diff": differences["honor"],
                "crystal_diff": differences["crystal"],
                "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            }

            customer["history"].append(result)
...
# --- History Tab ---
with tab2:
    st.subheader("📜 Service History Log")
